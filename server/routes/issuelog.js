...
router.get('/', authorization, userCheck, async (req, res) => {
  try {
    const data = await db.query("WITH requester_name AS (SELECT id, username FROM users),\
                                ...
                                SELECT issue_log.id, requester_name.username as req_name, created_on, status, type, subtype, comment, trans_desc_id, sentence_id, audio_name.username as ad_name, language from issue_log\
...
router.get('/:id', authorization, userCheck, async (req, res) => {
  ...
  try {
    ...
    const data = await db.query("WITH requester_name AS (SELECT id, username FROM users),\
                                audio_name AS (SELECT id, username FROM users)\
                                SELECT issue_log.id, requester_name.username as req_name, created_on, status, type, subtype, comment, issue_log.trans_desc_id, issue_log.sentence_id, audio_name.username as ad_name, language, trans_text.data as transtext, blobtest.data as audiodata from issue_log\
